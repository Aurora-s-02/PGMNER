...
def eval_std_f1_score(features, set_type, invalid_gt_num=0):
    ...
    for feature in features:
        for (roles, gt_dict_word, pred_dict_word) in zip(feature.list_roles, feature.gt_dict_word, feature.pred_dict_word):
            ...
            pred_dict['预测实体范围list'] = all_pred_list
            pred_dict['真实实体范围list'] = all_gt_list
            pred.append(pred_dict)
    if set_type == 'TEST':
        try:
            with open('pred_output_TEST_food.txt', 'a', encoding='utf-8') as file:
                if file.tell() != 0:
                    file.write("\n\n")  # 如果文件不为空，先插入两个空行
                file.write("\n".join(map(str, pred)))  # 将列表中的值转换为字符串并逐行写入文件
                print("Data written to file successfully.")
        except Exception as e:
            print("An error occurred while writing to the file:", e)
    elif set_type == 'DEV':
        try:
            with open('pred_output_DEV_food.txt', 'a', encoding='utf-8') as file:
                if file.tell() != 0:
                    file.write("\n\n")  # 如果文件不为空，先插入两个空行
                file.write("\n".join(map(str, pred)))  # 将列表中的值转换为字符串并逐行写入文件
                print("Data written to file successfully.")
