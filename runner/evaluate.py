...
class Evaluator(BaseEvaluator):
    ...
    def convert_batch_to_inputs(self, batch):
        inputs = {
            ...
            'image_input_ids':          batch[2].to(self.cfg.device),
            'aux_image_input_ids':      batch[3].to(self.cfg.device),
            'dec_table_ids':            batch[4].to(self.cfg.device),
            'dec_table_attention_mask': batch[5].to(self.cfg.device),
            'dec_prompt_lens':          batch[6],
            'list_target_info':         None,
            'old_tok_to_new_tok_indexs':batch[8],
            'list_arg_slots':           batch[9],
            'list_roles':               batch[10],
            'trigger_enc_token_index':  batch[13],
            'list_arg_2_prompt_slots':  batch[14],
            'cum_event_nums_per_type':  batch[15],
            'list_dec_prompt_ids':      batch[16].to(self.cfg.device),
            'list_len_prompt_ids':      batch[17].to(self.cfg.device),
        }
        named_v = {
            "list_roles":               batch[10],
            "feature_ids":              batch[12],
        }
    ...
    def predict(self):
        ...
        for metric, eval_fn in self.metric_fn_dict.items():
            perf_c, perf_i = eval_fn(self.features, self.set_type, self.invalid_num)
