...
class Events:
    def __init__(self, doc_id, context, event_type_2_events, image, aux_image):
        self.doc_id = doc_id
...
class DSET_processor:
    def __init__(self, args, tokenizer, transform):
        self.args = args
    ...
    def _create_example(self, lines, file_path, aux_image_path, aux_image, image_path, over_sample=None, max_num_event=MAX_NUM_EVENTS):
        W = self.args.window_size

        examples = []
        for line in lines:
            doc_id = line["id"]
            image_id = doc_id.split('_')[0] + '.jpg'
            ...
            if image_path is not None:
                ...
                try:  # 本身的图片
                    ...
                    image = self.transform(image)  # 图片的input
                except:
                    img_path = os.path.join(image_path, 'inf.png')
                    image = Image.open(img_path).convert('RGB')
                    image = self.transform(image)

                if aux_image_path is not None:  # 目标识别后的图片
                    aux_imgs = []
                    aux_img_paths = []
                    if image_id in aux_image:
                        aux_img_paths = aux_image[image_id]
                        aux_img_paths = [os.path.join(aux_image_path, path) for path in aux_img_paths]
            ...
            if self.args.single:
                for event in events:
                    ...
                    examples.append(Events(doc_id, context, event_type_2_events, image, aux_imgs))
            ...
            if over_sample == 'double' and num_events > 1:
                ...
                for i, events_ in enumerate(power_set):
                    ...
                    examples.append(Events('%d-%s' % (i, doc_id), context, event_type_2_events, image, aux_imgs))
                # 图片进入
    ...
    def create_example(self, file_path, aux_image_path, imgspath, image_path, set_type):
        ...
        elif self.args.dataset_type=='flickr30k':
            return self._create_example(lines,  file_path, aux_image_path, imgspath, image_path,over_sample=('double' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_furn':
            return self._create_example(lines, file_path, aux_image_path, imgspath, image_path, over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_Animals':
            return self._create_example(lines, file_path, aux_image_path, imgspath, image_path, over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_Archlnfra':
            return self._create_example(lines, file_path, aux_image_path, imgspath, image_path, over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_ArtDecor':
            return self._create_example(lines, file_path, aux_image_path, imgspath, image_path, over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_ClothingAcc':
            return self._create_example(lines, file_path, aux_image_path, imgspath, image_path, over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_ElecEntertain':
            return self._create_example(lines, file_path, aux_image_path, imgspath, image_path, over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_Food':
            return self._create_example(lines, file_path, aux_image_path, imgspath, image_path, over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_Misc':
            return self._create_example(lines, file_path, aux_image_path, imgspath, image_path, over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_Nature':
            return self._create_example(lines,  file_path, aux_image_path, imgspath, image_path,over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_People':
            return self._create_example(lines,  file_path, aux_image_path, imgspath, image_path,over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_SignComm':
            return self._create_example(lines, file_path, aux_image_path, imgspath, image_path, over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_Sport':
            return self._create_example(lines, file_path, aux_image_path, imgspath, image_path, over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_TransVeh':
            return self._create_example(lines, file_path, aux_image_path, imgspath, image_path, over_sample=('power' if set_type=='train' else None))
        elif self.args.dataset_type=='flickr30k_test_food':
            return self._create_example(lines, file_path, aux_image_path, imgspath, image_path, over_sample=('double' if set_type=='train' else None))
        else:
            raise NotImplementedError()
    
    def convert_examples_to_features(self, examples, transform):
        features = []
    ...
    def load_auximage(self, aux_path):
        aux_imgs = torch.load(aux_path)
        return aux_imgs

    def generate_dataloader(self, set_type, transform):
        assert (set_type in ['train', 'dev', 'test'])
        if set_type=='train':
            file_path = self.args.train_file
            aux_image_path = self.args.train_aux
            aux_image = self.load_auximage(self.args.train_imgspath)
            image_path = self.args.image
        elif set_type=='dev':
            file_path = self.args.dev_file
            aux_image_path = self.args.dev_aux
            aux_image = self.load_auximage(self.args.dev_imgspath)
            image_path = self.args.image
        else:
            file_path = self.args.test_file
            aux_image_path = self.args.test_aux
            aux_image = self.load_auximage(self.args.test_imgspath)
            image_path = self.args.image
        
        examples = self.create_example(file_path, aux_image_path, aux_image, image_path, set_type)
        ...
        features = self.convert_examples_to_features(examples, transform, self.args.role_name_mapping)
