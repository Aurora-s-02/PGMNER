...
class InputFeatures(object):
    ...
    def __init__(self, example_id, feature_id, list_event_type, image, aux_image, list_event_trigger,
...
class ArgumentExtractionDataset(Dataset):
    ...
    def collate_fn(batch):
        ...
        image_input_ids = torch.stack([f.image for f in batch])
        aux_image_input_ids = torch.stack([f.aux_image for f in batch])
        ...
        return enc_input_ids, enc_mask_ids, \
                image_input_ids, aux_image_input_ids, \
                dec_table_ids, dec_table_attention_mask, dec_prompt_lens, \
...
class MultiargProcessor(DSET_processor):
    def __init__(self, args, tokenizer, transform):
        super().__init__(args, tokenizer, transform)
        self.set_dec_input()
    ...
    def convert_examples_to_features(self, examples, transform, role_name_mapping=None):
        ...
        for example in examples:
            ...
            feature = InputFeatures(example_id, feature_idx, list_event_type, image, aux_image, triggers,
                                enc_text, enc_input_ids, enc_mask_ids,
                                trigger_enc_token_index,
                                dec_table_ids, dec_table_attention_mask, dec_prompt_lens,
                                list_arg_slots, list_target_info,
